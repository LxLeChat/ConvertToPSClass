...
Class Entry {
    ...
    [void] ParseProperties () {
        foreach($Prop in $this.Object.PSObject.properties) {
            ...
            ## this might be called if whe have a "Someproperty" : null
            If ( ($Prop.TypeNameOfValue -match '^System.Object$') -and ($null -eq $Prop.Value) )
            {
                $This.SetNullObjectProperty($Prop)
                Continue
            }
        }
    }

    ## called if whe have a "Someproperty" : null
    [void] SetNullObjectProperty ([PSNoteProperty]$NoteProperty) {
        $this.Properties += [Property]::new($NoteProperty.Name,"Object",$false)
    }
